  ...
  const handleLogout = async () => {
    ...
    if (!error) {
      ...
      const response = window.confirm(
        "¿Hubo un error al cerrar Sesion, continuar de todos modos?"
      );
      if (response) {
        sessionStorage.removeItem("SESSID");
        history.go(0);
  ...
  return (
    <nav className="navbar">
      ...
      <div className="link_group">
        <Link className={"link " + linkStyles.events} to="/events">
          Eventos
        </Link>
        <Link className={"link " + linkStyles.asocs} to="/asocs">
          Asociaciones
        </Link>
        <Link className={"link " + linkStyles.users} to="/users">
          Usuarios
        </Link>
        <Link className={"link " + linkStyles.support} to="/support">
          Soporte
        </Link>
      </div>
